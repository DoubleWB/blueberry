...
class IKSolver:
    ...
    def moveToGoalAbsolute(point):
        ...
        self._current = self.setAnglesAndGrip(bestConfig, self._grip_neutral)

    def moveToGoalRelative(point):
        """Adjust robot by the given differential vector and record angles""" 
        pointAsVector = [point.x, point.y, point.z]
        x = self.getEE(q)
        x = x.col(-1)
        x.row_del(-1)
        relativePoint = x + pointAsVector
